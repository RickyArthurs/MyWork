...
public class FunEncoderVisitor extends AbstractParseTreeVisitor<Void> implements FunVisitor<Void> {
	...
	{
		List<FunParser.Seq_comContext> scoms = ctx.seq_com();
		List<FunParser.GuardContext> guard_list = ctx.guard();
		
		List<Integer> adrs = new ArrayList<Integer>();

		for(int i = 0; i < guard_list.size(); i++)
		{
			visit(ctx.expr());
			visit(guards.get(i));
			int g_addr = obj.currentOffset();

			obj.emit12(SVM.JUMPF,0);

			visit(scoms.get(i));
			int s_addr = obj.currentOffset();
			adrs.add(s_adr);
			
			obj.emit12(SVM.JUMP, 0);
			int nextaddr = obj.currentOffset();
			obj.patch12(g_addr, nextaddr);
		}

		visit(scoms.get(scoms.size())-1);
		// store exit address
		int exit = obj.currentOffset();
		for(int adr : adrs)
		{
			obj.patch12(adr, exit);
		}
		return null;
	}

	// Trueg
	public Void visitTrueg(FunParser.TruegContext ctx)
	{
		obj.emit12(SVM.LOADC, 1);
	...
	// Falseg
	public Void visitFalseg(FunParser.FalsegContext ctx)
	{
		obj.emit12(SVM.LOADC,0);
		obj.emit12(SVM.COMPG);

		return null;
	}

	// Numg
	public Void visitNumg(FunParser.NumgContext ctx)
	{
		int num = Integer.parseInt(ctx.NUM().getText());
		obj.emit12(SVM.LOADC, num);
		obj.emit1(SVM.COMPG);
		return null;
	}

	// Rangeg
	public Void visitRangeg(FunParser.RangegContext ctx)
	{
		int a = Integer.parseInt(ctx.NUM(0).getText());
		int b = Integer.parseInt(ctx.NUM(1).getText());

		obj.emit12(SVM.LOADC,a);
		obj.emit12(SVM.LOADC,b);
		obj.emit1(SVM.LOADR);
