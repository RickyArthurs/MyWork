...
//
//Development of for and switch procedures
//
//Richard Arthurs - 2465714A - 19/03/2022
//
//////////////////////////////////////////////////////////////
...
public class FunCheckerVisitor extends AbstractParseTreeVisitor<Type> implements FunVisitor<Type> {
	...
	public Type visitFor(FunParser.ForContext ctx){
		...
		Type type1 = visit(ctx.e1);
		Type type2 = visit(ctx.e2);
		visit(ctx.seq_com());
		checkType(Type.INT, type1, ctx);
		checkType(Type.INT, type2, ctx);

		return null;

	}
	// switch visit method
		public Type visitSwitch(FunParser.SwitchContext ctx) {                                //EXTENSION
		//get type
		Type exptype = visit(ctx.expr());
		//check type
		if(exptype != Type.BOOL){
			reportError("Invalid data type", ctx);
		}
		if(exptype != Type.INT)
		{
			reportError("Invalid data type", ctx);
		...
		// checked
		List<String> checked_guards = new ArrayList<String>();
		//intialise guard
		List<FunParser.GuardContext> guards = ctx.guard();

		// check for no of guards
		for(FunParser.GuardContext guard : guards){
			//validate each uteration
			Type gtype = visit(guard);
			checkType(gtype, exptype, ctx);
			// check for duplicates
			if(checked_guards.contains(guard.getText())){
				reportError("Duplicate guards", ctx);
			} 
			else if(guard_type == Type.BOOL) {
				//after validation append
				checkType(guard_type, expr_type, ctx);
				checked_guards.add(guard.getText());
			} 
			else{ //integer case
				
				if (guard.getText().matches("\\d+")) { //if in valid range ie. 0-9 and next digit is ascending

					checkType(guard_type, expr_type, ctx);
					checked_guards.add(guard.getText());
				}
				else{
					String[] scope = (guard.getText()).split("\\..");
					//compare digits
					int a = Integer.parseInt(scope[0].replaceAll("[\\D]", ""));
					int b = Integer.parseInt(scope[1].replaceAll("[\\D]", ""));
					if (a>=b) {
						reportError("Digits are not ascending ", ctx);
					}
					for(String str : checked_guards) {
						// check digit values
						if (b >= Integer.parseInt(str) && a <= Integer.parseInt(str)){
							reportError("Overlapped guards ", ctx);
						}
					}
					for (int i = a; i <= b; i++){
						checked_guards.add(Integer.toString(i));
					}
				}
			}
	...
	public Type visitTrueg(FunParser.TruegContext ctx)// visit Trueg guard
	{
		return Type.BOOL;
	}

	public Type visitFalseg(FunParser.FalsegContext ctx)// visit FalseG guard
	{
		return Type.BOOL;
	}

	public Type visitNumg(FunParser.NumgContext ctx)// visit Numg guard
	{
		return Type.INT;
	}

	public Type visitRangeg(FunParser.RangegContext ctx)// visit RangeG guard
	{
		return Type.INT;
	}
