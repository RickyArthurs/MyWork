...
//#include "C:\Program Files\NVIDIA GPU Computing Toolkit\CUDA\v11.6\include\CL\cl.h"
#include <CL/cl.h>
#else
...
CLObject *init_driver()
{
    ...
    if (err != 0)
    {
        fprintf(stderr, "Failed to create mutex %d", err);
        exit(EXIT_FAILURE);
    }
...
int shutdown_driver(CLObject *ocl)
{
    ...
    if (err != 0)
    {
        fprintf(stderr, "Failed to destory the mutex %d", err);
        exit(EXIT_FAILURE);
    }
    // END of assignment code section
    //===============================================================================================================================================================
...
int run_driver(CLObject *ocl, unsigned int buffer_size, int *input_buffer_1, int *input_buffer_2, int w1, int w2, int *output_buffer)
{
    ...
    // Counter for no of iterations thus far
    int counter = 0;

    // Create mutex for driver
    err = pthread_mutex_lock(&ocl->device_lock);
    if (err != 0)
    {
        fprintf(stderr, "Failed to create the mutex %d", err);
        exit(EXIT_FAILURE);
    }
    // Create the buffer objects to link the input and output arrays in device memory to the buffers in host memory

    input1 = clCreateBuffer(ocl->context, CL_MEM_READ_ONLY, sizeof(int) * buffer_size, NULL, &err);
    if (input1 == NULL)
    {
        fprintf(stderr, "input buffer error - %d!\n", err);
        exit(EXIT_FAILURE);
    }
    input2 = clCreateBuffer(ocl->context, CL_MEM_READ_ONLY, sizeof(int) * buffer_size, NULL, &err);
    if (input2 == NULL)
    {
        fprintf(stderr, "input buffer error - %d!\n", err);
        exit(EXIT_FAILURE);
    }
    output = clCreateBuffer(ocl->context, CL_MEM_WRITE_ONLY, sizeof(int) * buffer_size, NULL, &err);
    if (output == NULL)
    {
        fprintf(stderr, "input buffer error - %d!\n", err);
        exit(EXIT_FAILURE);
    }
    status_buf = clCreateBuffer(ocl->context, CL_MEM_READ_ONLY, sizeof(status) * buffer_size, NULL, &err);
    if (status_buf == NULL)
    {
        fprintf(stderr, "input buffer error - %d!\n", err);
        exit(EXIT_FAILURE);
    ...
    err = clEnqueueWriteBuffer(ocl->command_queue, input1, CL_TRUE, 0, sizeof(int) * buffer_size, input_buffer_1, 0, NULL, NULL);
    if (err != CL_SUCCESS)
    {
        fprintf(stderr, "Failed to write input1 into device memory - %d!\n", err);
        exit(EXIT_FAILURE);
    }

    err = clEnqueueWriteBuffer(ocl->command_queue, input2, CL_TRUE, 0, sizeof(int) * buffer_size, input_buffer_2, 0, NULL, NULL);
    if (err != CL_SUCCESS)
    {
        fprintf(stderr, "Failed to write input2 into device memory - %d!\n", err);
        exit(EXIT_FAILURE);
    }
    ...
    if (err != CL_SUCCESS)
    {
        fprintf(stderr, "Failed to assign kernal argument index 0 - %d!\n", err);
        exit(EXIT_FAILURE);
    }
    err = clSetKernelArg(ocl->kernel, 1, sizeof(input2), &input2);
    if (err != CL_SUCCESS)
    {
        fprintf(stderr, "Failed to assign kernal argument index 1 - %d!\n", err);
        exit(EXIT_FAILURE);
    }
    err = clSetKernelArg(ocl->kernel, 2, sizeof(output), &output);
    if (err != CL_SUCCESS)
    {
        fprintf(stderr, "Failed to assign kernal argument index 2 - %d!\n", err);
        exit(EXIT_FAILURE);
    }
    err = clSetKernelArg(ocl->kernel, 3, sizeof(status_buf), &status_buf);
    if (err != CL_SUCCESS)
    {
        fprintf(stderr, "Failed to assign kernal argument index 3 - %d!\n", err);
        exit(EXIT_FAILURE);
    }
    err = clSetKernelArg(ocl->kernel, 4, sizeof(w1), &w1);
    if (err != CL_SUCCESS)
    {
        fprintf(stderr, "Failed to assign kernal argument index 4 - %d!\n", err);
        exit(EXIT_FAILURE);
    }
    err = clSetKernelArg(ocl->kernel, 5, sizeof(w2), &w2);
    if (err != CL_SUCCESS)
    {
        fprintf(stderr, "Failed to assign kernal argument index 5 - %d!\n", err);
        exit(EXIT_FAILURE);
    }
    err = clSetKernelArg(ocl->kernel, 6, sizeof(buffer_size), &buffer_size);
    if (err != CL_SUCCESS)
    {
        fprintf(stderr, "Failed to assign kernal argument index 6 - %d!\n", err);
        exit(EXIT_FAILURE);
    }

    // Execute the kernel, i.e. tell the device to process the data using the given global and local ranges

    // Allow to execute once before checking conditions
    do
    {
        cl_event kernel_event;
        err = clEnqueueNDRangeKernel(ocl->command_queue, ocl->kernel, (cl_uint)1, NULL, &global, NULL, 0, NULL, &(kernel_event));
        if (err != CL_SUCCESS)
        {
            fprintf(stderr, "Kernel enqueue has failed - %d!\n", err);
            continue;
        }

        // This operation "clEnqueueReadBuffer" ensures buffer waits until kernel_event has been completed before executing
        ...
        // Check the status
        err = clEnqueueReadBuffer(ocl->command_queue, status_buf, CL_TRUE, 0, sizeof(status), status, 1, &(kernel_event), NULL);
        if (err != CL_SUCCESS)
        {
            fprintf(stderr, "Enqueue has failed and status not read - %d!\n", err);
            continue;
        }
    } while (counter++ < max_iters && status[0] != 0); //check conditions before looping - status is not 0 i.e. work queue complete and max_iters not exceeded if specified

    // When the status is 0, read back the results from the device to verify the output

    if (status[0] != 0)
    {
        fprintf(stderr, "Process has failed - %d!\n", err);
        exit(EXIT_FAILURE);
    }
    ...
    if (err != CL_SUCCESS)
    {
        fprintf(stderr, "Final result could not be read %d\n", err);
        exit(EXIT_FAILURE);
    }
    ...
    if (err != CL_SUCCESS)
    {
        fprintf(stderr, "Buffer deletion failed for input1 %d\n", err);
        exit(EXIT_FAILURE);
    }
    err = clReleaseMemObject(input2);
    if (err != CL_SUCCESS)
    {
        fprintf(stderr, "Buffer deletion failed for input2 %d\n", err);
        exit(EXIT_FAILURE);
    }
    err = clReleaseMemObject(output);
    if (err != CL_SUCCESS)
    {
        fprintf(stderr, "Buffer deletion failed for output %d\n", err);
        exit(EXIT_FAILURE);
    }
    err = clReleaseMemObject(status_buf);
    if (err != CL_SUCCESS)
    {
        fprintf(stderr, "Buffer deletion failed for status_buf %d\n", err);
        exit(EXIT_FAILURE);
    }

    // now release relevant mutex

    err = pthread_mutex_unlock(&ocl->device_lock);
    if (err != 0)
    {
        fprintf(stderr, "Mutex unsuccessfully released %d\n", err);
        exit(EXIT_FAILURE);
    }
    // END of assignment code section
    //===============================================================================================================================================================
